...
describe "/environments/deltacloud.xml.haml" do
  ...
  before(:each) do
    ...
    @environment.stub_chain(:cloud_profile, :cloud, :name).and_return('the-cloud')
    @environment.stub_chain(:cloud_profile, :cloud, :hardware_profiles_url).and_return('http://test.server/hardware_profiles')
    @environment.stub_chain(:cloud_profile, :cloud, :realms_url).and_return('http://test.server/realms')
    @environment.stub_chain(:cloud_profile, :cloud, :instance_states_url).and_return('http://test.server/instance_states')
    @environment.stub_chain(:cloud_profile, :cloud, :images_url).and_return('http://test.server/images')
    @environment.stub_chain(:cloud_profile, :cloud, :instances_url).and_return('http://test.server/instances')
  end

  it "renders the DeltaCloud API endpoint for the enironment" do
    @environment.cloud.should_receive(:name).and_return('the-cloud')
    render
